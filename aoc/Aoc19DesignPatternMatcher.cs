...
public class Aoc19DesignPatternMatcher
{
    ...
    {
        var m = DesignPatternMatcher.FromString(Test1);
        var matches = m.FindMatches();
        var count = matches.Count(p => p > 0);
    ...
    {
        var m = DesignPatternMatcher.FromString(Input1);
        var matches = m.FindMatches();
        var count = matches.Count(p => p > 0);
        TestContext.Out.WriteLine(count);
        Assert.That(count, Is.EqualTo(240));
    }
    ...
    public void Solution2()
    {
        ...
        var sum = matches.Sum();
        TestContext.Out.WriteLine(sum);
        Assert.That(sum, Is.EqualTo(848076019766013));
    }

...
[TestFixture]
public class Aoc2LevelReports
{
    private const string Simple1 = """
                                   7 6 4 2 1
                                   1 2 7 8 9
                                   9 7 6 2 1
                                   1 3 2 4 5
    ...
    {
        var report = LevelReports.FromString(Simple1);
        var safe = report.SafeReports(); 
        Assert.That(safe.Sum(x=>x ? 1 : 0), Is.EqualTo(2));
    }
    ...
    {
        var report = LevelReports.FromString(Simple1);
        var safe = report.SafeReportsWithDampening(); 
        Assert.That(safe.Sum(x=>x ? 1 : 0), Is.EqualTo(4));
    }
    ...
    {
        var report = LevelReports.FromString(Input1);
        var safe = report.SafeReports();
        var sum = safe.Sum(x => x ? 1 : 0);
